...
export default function Gallery({ navigation }) {
  ...
  const showCards= item => <CardsGallery key={item._id} item={item}  handleShowModal={handleModal} handleSharing={handleSharing} handleBasket={handleBasket} handleSelected={handleSelected}/>
  const tempGallery=  galleryImages.reverse().map((element) => 
      element.imageResult.filter(item=>item.isChecked === isFavoriteView).map(showCards)
  ).flat();
  ...
  return (
    <View style={tw`flex-1 bg-[#F2EFEA] items-center`}>
      ...
      <SwitchControl handleSwitchParent={handleSwitchGallery} currentValue={isFavoriteView}  title='Show liked onlyyy' />
